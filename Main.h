...
template<typename ValueType, typename SizeType>
long long InsertionSort(ValueType*& _array, const SizeType _arraySize)
{
    long long _operationCount = 0; // Compter le Nombre d'Opération
    for (SizeType _index = 0; _index < _arraySize; _index++)
    {
        ValueType _x = _array[_index];
...
void InsertionSortDemo(const bool _wantToDisplayArray)
{
    ...
    cout << RED << endl;
    cout << "Tri par Insertion en cours..." << endl;

    using Time = steady_clock::time_point;
    const Time& _start = high_resolution_clock::now();

    long long _operationCount = InsertionSort<ValueType, SizeType>(_array, _arraySize);
    ...
    system("pause");
    system("cls");
    InsertionSortDemo<ValueType, SizeType>(_wantToDisplayArray);
}
