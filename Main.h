...
template<typename ValueType, typename SizeType>
long long BubbleSort(ValueType*& _array, const SizeType _arraySize)
{
    long long _operationCount = 0; // Compter le Nombre d'Opération
    for (SizeType _index = _arraySize; _index > 1; _index--)
    {
        bool _isArraySorted = true;
        _operationCount++; // Compter le Nombre d'Opération

        for (SizeType _secondIndex = 0; _secondIndex < _index - 1; _secondIndex++)
        {
            // < ordre croissant
            // > ordre décroissant
            if (_array[_secondIndex + 1] < _array[_secondIndex])
            {
                Swap<ValueType>(_array, _secondIndex, _secondIndex + 1);
                _isArraySorted = false;
                _operationCount += 4; // Compter le Nombre d'Opération
            }
            _operationCount++; // Compter le Nombre d'Opération
        }
        if (_isArraySorted) return _operationCount;
        _operationCount++; // Compter le Nombre d'Opération
    }
    return -1; // Sécurité
}
...
void BubbleSortDemo(const bool _wantToDisplayArray)
{
    ...
    cout << RED << endl;
    cout << "Tri à Bulle en cours..." << endl;

    using Time = steady_clock::time_point;
    const Time& _start = high_resolution_clock::now();

    long long _operationCount = BubbleSort<ValueType, SizeType>(_array, _arraySize);
    ...
    system("pause");
    system("cls");
    BubbleSortDemo<ValueType, SizeType>(_wantToDisplayArray);
}
