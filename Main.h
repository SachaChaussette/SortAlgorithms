...
#pragma region OddEvenSort

template<typename ValueType, typename SizeType>
long long OddEvenSort(ValueType*& _array, const SizeType& _arraySize)
{
    ...
    while (!_sorted)
    {
        ...
        for (u_int _index = 0; _index < _arraySize - 1; _index += 2)
        {
            // < ordre décroissant
            // > ordre croissant 
        ...
        for (u_int _index = 1; _index < _arraySize - 1; _index += 2)
        {
            // < ordre décroissant
            // > ordre croissant 
            if (_array[_index] > _array[_index + 1])
...
void OddEvenSortDemo(const bool _wantToDisplayArray)
{
    ...
    long long _operationCount = OddEvenSort<ValueType, SizeType>(_array, _arraySize);
    ...
    system("pause");
    system("cls");
    OddEvenSortDemo<ValueType, SizeType>(_wantToDisplayArray);
}
