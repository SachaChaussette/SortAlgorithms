...
#pragma region CoktailShakerSort

template<typename ValueType, typename SizeType>
long long CocktailShakerSort(ValueType*& _array, const SizeType& _arraySize)
{
    ...
    while (_changed)
    {
        ...
        if (_arraySize == 1 || _arraySize == 0) return 0;

        for (u_int _index = 0; _index < _arraySize - 2; _index++)
        {
            // > pour l'ordre croissant
            // < pour l'ordre croissant
            if (_array[_index] > _array[_index + 1])
            {
                Swap<ValueType>(_array, _index, _index + 1);
        ...
        for (u_int _index = _arraySize - 2; _index > 0; _index--)
        {
            // > pour l'ordre croissant
            // < pour l'ordre croissant
            if (_array[_index] > _array[_index + 1])
...
void CoktailShakerSortDemo(const bool _wantToDisplayArray)
{
    ...
    long long _operationCount = CocktailShakerSort<ValueType, SizeType>(_array, _arraySize);
    ...
    system("pause");
    system("cls");
    CoktailShakerSortDemo<ValueType, SizeType>(_wantToDisplayArray);
}
